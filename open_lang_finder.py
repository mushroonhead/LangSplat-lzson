...
class ImageRelevancyPipeline(torch.nn.Module):
    ...
    """
    def __init__(self, 
                 dataset_params: GroupParams,
                 encoder_hidden_dims: Iterable[int], decoder_hidden_dims: Iterable[int],
                 auto_encoder_chkpt: dict,
                 device: torch.device,
                 ) -> None:
        super().__init__()
        ...
        self.autoencoder = Autoencoder(encoder_hidden_dims, decoder_hidden_dims).to(device)
        self.autoencoder.load_state_dict(auto_encoder_chkpt)
        # CLIP encoder
