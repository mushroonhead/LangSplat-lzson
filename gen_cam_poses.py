    ...
    for i in range(1, num_poses_per_side + 1):
        ...
        for direction in [-1, 0, 1]:  # -1 for left, 1 for right
            for direction2 in [-1, 0, 1]:  # -1 for down, 1 for up
                ...
                # offset2 = np.array([np.sin(angle_rad) * radius, -np.cos(angle_rad) * radius, 0])
                print(offset)
                # print(offset2)
                # offset += offset2
...
poses = generate_symmetric_camera_poses(initial_pose, center, num_poses_per_side, angle_change, radius)
